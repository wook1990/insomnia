    ...
    for i in team_name:
        ...
        if len(page_num) == 1:
           ...
           # driver.implicitly_wait(30)
            ...
            #player_record_df.to_csv("~/Desktop/data/KBO/2019_hitter_basic_1(cp949).csv",index=False, encoding = 'cp949')
...
if __name__ == "__main__" :
    ...
    '''
    '''
    data_url = [ "url_hitter2_basic", "url_hitter_detail", "url_pitcher_basic", "url_pitcher2_basic",
                "url_pitcher_detail", "url_pitcher2_detail", "url_defense", "url_runner"]
    '''
    data_url = ["url_runner"]
    ...
    for url in data_url:
        ...
        driver = session_open(chromdriver, eval(url))
        time.sleep(2)
        for i in year:
            line_logging(str(i)  + "년의 데이터를 크롤링 중입니다.")
            df = get_player_info(i)
            df["year"] = i
            df.to_csv("~/Desktop/data/KBO/{0}_{1}(cp949).csv".format(i, url.split("url_")[1]), index=False, encoding="cp949")
